...
public class Jugantor {
	...
	public void createPsot() throws IOException {
		...
		for (String id : postId) {
			...
			for (int i = 0; i < articleParas.size() - 2; i++) {
				...
				System.out.println(findImgUrlAndCaption(articleParas.get(i)));
	...
	public static HashSet<String> findPostIds() throws IOException {
		...
		Document document = Jsoup.connect("https://www.jugantor.com/").get();
		Element body = document.body();
		
		Element posts = body.getElementById("latest_list_block");
